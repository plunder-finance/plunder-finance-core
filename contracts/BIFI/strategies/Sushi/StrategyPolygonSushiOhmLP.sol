...
contract StrategyPolygonSushiOhmLP is StratManager, FeeManager {
    ...
    function callReward() public view returns (uint256) {
        ...
        if (rewarder != address(0)) {
            pendingOhm = IRewarder(rewarder).pendingToken(poolId, address(this));
        }

        try IUniswapRouterETH(unirouter).getAmountsOut(pendingOhm, rewardToOutputRoute)
                returns (uint256[] memory amountOut)
            {
                moreOutput = amountOut[amountOut.length -1];
            }
            catch {}

        uint256 outputBal = rewardsAvailable().add(moreOutput);
