...
contract StrategyCommonMiniChefMultiRewardsLP is StratManager, FeeManager {
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ICommonMiniChef(chef).withdraw(poolId, _amount.sub(wantBal), address(this));
    ...
    function rewardsAvailable() public view returns (uint256, uint256[] memory) {
        uint256 pngReward = ICommonMiniChef(chef).pendingReward(poolId, address(this));
