...
import "../../interfaces/finn/IFinnBar.sol";
import "../Common/StratManager.sol";
import "../Common/FeeManager.sol";
import "../../utils/StringUtils.sol";

contract StrategyFinn is StratManager, FeeManager {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IFinnBar(bar).deposit(wantBal);
            IMasterChef(chef).deposit(poolId, balanceOfBar());
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMasterChef(chef).withdraw(poolId, tokenForBarToken(_amount.sub(wantBal)));
            IFinnBar(bar).withdraw(balanceOfBar());
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 barBal,) = IMasterChef(chef).userInfo(poolId, address(this));
        uint256 _amount = barTokenForToken(barBal);
        return _amount;
    }
