...
import "../../interfaces/scream/ISteakHouse.sol";
import "../Common/StratManager.sol";
import "../Common/FeeManager.sol";

contract StrategySteakHouseLP is StratManager, FeeManager {
    ...
    function deposit() public whenNotPaused {
        require(ISteakHouse(chef).depositFee() <= 5000, "! Deposit Fee too high");
        uint256 wantBal = IERC20(want).balanceOf(address(this));

        if (wantBal > 0) {
            ISteakHouse(chef).deposit(poolId, wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ISteakHouse(chef).withdraw(poolId, _amount.sub(wantBal));
    ...
    function balanceOfPool() public view returns (uint256) {
        (,uint256 _amount) = ISteakHouse(chef).getUserInfo(poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        ISteakHouse(chef).emergencyWithdraw(poolId);
