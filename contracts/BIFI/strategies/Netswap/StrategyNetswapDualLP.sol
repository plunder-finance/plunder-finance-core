...
import "./INetswapChef.sol";
import "../Common/StratManager.sol";
import "../Common/FeeManager.sol";

contract StrategyNetswapDualLP is StratManager, FeeManager {
    ...
    constructor(
        ...
        require(_rewardToNativeRoute[_rewardToNativeRoute.length - 1] == native, "rewardToNativeRoute[last] != native");
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            INetswapChef(chef).deposit(poolId, wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            INetswapChef(chef).withdraw(poolId, _amount.sub(wantBal));
    ...
    function _harvest(address callFeeRecipient) internal {
        INetswapChef(chef).deposit(poolId, 0);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (rewardBal > 0 && reward != native) {
            IUniswapRouter(unirouter).swapExactTokensForTokens(rewardBal, 0, rewardToNativeRoute, address(this), now);
        }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,) = INetswapChef(chef).userInfo(poolId, address(this));
        return _amount;
    }

    function rewardsAvailable() public view returns (uint256) {
        (uint256 outputBal,,,) = INetswapChef(chef).pendingTokens(poolId, address(this));
        return outputBal;
    }

    function callReward() public view returns (uint256) {
        uint256 outputBal = rewardsAvailable();
        (,,,uint256 rewardBal) = INetswapChef(chef).pendingTokens(poolId, address(this));
        uint256 nativeBal;

        uint256[] memory amountOutFromOutput = IUniswapRouter(unirouter).getAmountsOut(outputBal, outputToNativeRoute);
        nativeBal = nativeBal.add(amountOutFromOutput[amountOutFromOutput.length - 1]);

        if (reward != native) {
            uint256[] memory amountOutFromReward = IUniswapRouter(unirouter).getAmountsOut(rewardBal, rewardToNativeRoute);
            nativeBal = nativeBal.add(amountOutFromReward[amountOutFromReward.length - 1]);
        } else {
            nativeBal = nativeBal.add(rewardBal);
        }
    ...
    function retireStrat() external {
        ...
        INetswapChef(chef).emergencyWithdraw(poolId);
    ...
    function swapRewardRoute(address[] calldata _rewardToNativeRoute) external onlyOwner {
        if (reward != native || reward != lpToken0 || reward != lpToken1) {
