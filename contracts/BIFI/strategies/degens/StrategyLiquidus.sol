...
contract StrategyLiquidus is StratManager, FeeManager, GasThrottler {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            ILiquidusStaking(chef).deposit(wantBal, address(this));
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ILiquidusStaking(chef).withdraw(_amount.sub(wantBal), address(this));
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ILiquidusStaking(chef).harvest(address(this));
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,,) = ILiquidusStaking(chef).userInfo(address(this));
        return _amount;
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        return ILiquidusStaking(chef).pendingReward(address(this));
    }
    ...
    function retireStrat() external {
        ...
        ILiquidusStaking(chef).emergencyWithdraw(address(this));
