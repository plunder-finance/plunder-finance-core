...
contract StrategyChargeLP is StratManager, FeeManager {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IMasterCharge(chef).deposit(wantBal, poolId);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMasterCharge(chef).withdraw(_amount.sub(wantBal), poolId);
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IMasterCharge(chef).deposit(0, poolId);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,) = IMasterCharge(chef).userInfo(poolId, address(this));
        return _amount;
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        return IMasterCharge(chef).pendingReward(address(this), poolId);
    }
    ...
    function retireStrat() external {
        ...
        IMasterCharge(chef).emergencyWithdraw(poolId);
