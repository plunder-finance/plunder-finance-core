...
import "../../interfaces/yode/IYodeChefV1.sol";
...
contract StrategyYodeChefLP is StratManager, FeeManager, GasThrottler {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IYodeChefV1(chef).deposit(poolId, wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IYodeChefV1(chef).withdraw(poolId, _amount.sub(wantBal));
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IYodeChefV1(chef).deposit(poolId, 0);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,,,) = IYodeChefV1(chef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function rewardsAvailable() public view returns (uint256) {
        uint256 outputBal = IYodeChefV1(chef).pendingYode(poolId, address(this));
        return outputBal;
    }
    ...
    function retireStrat() external {
        ...
        IYodeChefV1(chef).emergencyWithdraw(poolId);
