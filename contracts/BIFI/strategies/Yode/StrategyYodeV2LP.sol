...
import "../../interfaces/yode/IYodeChefV2.sol";
import "../../interfaces/spooky/ISpookyRewarder.sol";
import "../Common/StratManager.sol";
import "../Common/FeeManager.sol";

contract StrategyYodeV2LP is StratManager, FeeManager {
    ...
    function deposit() public whenNotPaused payable {
        ...
        if (wantBal > 0) {
            IYodeChefV2(chef).deposit(poolId, wantBal);
    ...
    function withdraw(uint256 _amount) external payable {
        ...
        if (wantBal < _amount) {
            IYodeChefV2(chef).withdraw(poolId, _amount.sub(wantBal));
    ...
    function _harvest(address callFeeRecipient) internal {
        IYodeChefV2(chef).deposit(poolId, 0);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, , , ) = IYodeChefV2(chef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        IYodeChefV2(chef).emergencyWithdraw(poolId);
