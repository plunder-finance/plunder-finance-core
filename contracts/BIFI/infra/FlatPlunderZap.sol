...
// File: contracts/BIFI/zap/PlunderUniV2Zap.sol
...
interface IPlunderVaultV6 is IERC20 {
    function deposit(uint256 amount) external;
    function withdraw(uint256 shares) external;
    function want() external pure returns (address);
}

contract PlunderUniV2Zap {
    using LowGasSafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for IPlunderVaultV6;
    ...
    function beefOutAndSwap(address plunderVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        ...
        require(token0 == desiredToken || token1 == desiredToken, 'Plunder: desired token not present in liqudity pair');
    ...
    function _getVaultPair (address plunderVault) private view returns (IPlunderVaultV6 vault, IUniswapV2Pair pair) {
        vault = IPlunderVaultV6(plunderVault);
    ...
    function _swapAndStake(address plunderVault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        require(isInputA || pair.token1() == tokenIn, 'Plunder: Input token not present in liqudity pair');
    ...
    function checkWETH() public view returns (bool isValid) {
        ...
        require(isValid, 'Plunder: WETH address not matching Router.WETH()');
    }
