...
contract VaultUser {
    ...
    function deposit(IPlunderVaultV6 vault_, uint256 amount_) external returns (uint256 mooShares_) {
        vault_.deposit(amount_);
        mooShares_ = vault_.balanceOf(address(this));
    }

    function depositAll(IPlunderVaultV6 vault_) external returns (uint256 mooShares_) {
        vault_.depositAll();
        mooShares_ = vault_.balanceOf(address(this));
    }

    function withdraw(IPlunderVaultV6 vault_, uint256 shares_) external returns (uint256 want_) {
        vault_.withdraw(shares_);
        want_ = IERC20Like(vault_.want()).balanceOf(address(this));
    }

    function withdrawAll(IPlunderVaultV6 vault_) external returns (uint256 want_) {
        vault_.withdrawAll();
        want_ = IERC20Like(vault_.want()).balanceOf(address(this));
